...
@php
    ...
    $params = array_merge($parsedParams, [
        ...
        'name' => $query, // Search by name/query
    ]);
...
<x-ta-vpp-theme::layouts>
    ...
    <div class="container">
        ...
        {{-- Suggestions --}}
        @if ($suggestion)
            <div class="suggestion-message mb-4">
                <p>
                    {{ trans('shop::app.search.suggest') }}
                    <a href="{{ route('shop.search.index', ['query' => $suggestion, 'suggest' => 0]) }}" class="text-blue-600 hover:underline">
                        {{ $suggestion }}
                    </a>
                </p>
            </div>
        ...
        <div class="catalog-layout">
            ...
            <aside>
                <form id="filter-form">
                    ...
                    <div class="mt-4" id="clear-filters-btn" style="display: {{ request()->except(['query', 'page', 'sort', 'limit']) ? 'block' : 'none' }}">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function renderProductCard(product) {
                 ...
                 // If we had a clear discount (regular > min), we could show it.
                 // let regularPrice = product.prices.regular.formatted_price;
                 // let minPrice = product.min_price; // "$53.99"
                 
                 // Since we don't have easy logic to compare formatted strings without parsing,
                 // and the example provided doesn't show discount, we will stick to min_price.
                 // However, we can use price_html from API if we want to be lazy, but it might not match design.
